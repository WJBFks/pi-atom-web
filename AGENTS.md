...
- 长会话必须为屏外历史条目开启 `content-visibility:auto`（`#message-history>*` 配 `contain-intrinsic-size:auto 260px`）：否则改变内容列宽度或滚动会重排整份 transcript（实测 8000 节点约 40ms/帧，开启后约 7ms）。屏外条目使用上次实测尺寸、缺失时回退到估算值；Chrome 的滚动锚定保证滚动位置稳定（实测锚点偏差 ≤1px），自动跟随底部仍然精确。
...
- Thinking、工具调用、TUI 通知及自定义 Entry 必须复用 `DisclosureBlock` 的统一 `details/summary` 折叠结构；各业务组件只负责标题、状态、计时及正文。所有折叠摘要必须把连续空白和换行压成单个空格，只占一个视觉行并在溢出时显示省略号。
