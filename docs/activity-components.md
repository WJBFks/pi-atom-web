...
| Prompt 队列 | 500 | Steering 或 Follow-up 总数大于 0 | 否 |

Prompt 队列设置 `autoOpen:false`，其 tab 名称实时显示 `队列 N`；总数为 0 时不渲染，即使模型正在生成也不会显示“队列 0”。普通 Composer 提交的 Prompt 默认进入 Follow-up 排队。活动面板不提供第二个输入区，只管理已有项：上方纵向展示 Steering 引导，下方展示 Follow-up 排队；可把排队项转换为 Steering 以引导当前轮，也可转回排队，并支持编辑文本或删除。
...
扩展请求首次出现时，活动栏会自动展开尚未自动展示过的第一个请求。用户手动折叠后，不再自动打断用户；所有活动消失后会清空这份临时选择状态。`autoOpen:false` 的活动永远不参与自动展开候选。
